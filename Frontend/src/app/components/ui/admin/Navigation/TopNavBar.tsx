...
export default function TopNavBar({ isAdmin }: { isAdmin: boolean }) {
  ...
  const menuItems = useMemo((): MenuItem[] => {
    const baseItems: MenuItem[] = [
      { id: 1, label: "Записи", count: counts?.appointments ?? "-", href: ADMIN_ROUTES.APPOINTMENTS.LIST, icon: iconComponents.appointments, description: "Управление записями" },
      { id: 2, label: "Сотрудники", count: counts?.masters ?? "-", href: ADMIN_ROUTES.MASTERS.LIST, icon: iconComponents.masters, description: "Управление персоналом" },
      { id: 3, label: "Расписание", count: counts?.schedule ?? "-", href: ADMIN_ROUTES.SCHEDULE.OVERVIEW, icon: iconComponents.schedule, description: "График работы" },
      { id: 4, label: "Категории", count: counts?.category ?? "-", href: ADMIN_ROUTES.CATEGORY.LIST, icon: iconComponents.category, description: "Категории услуг" },
      { id: 5, label: "Услуги", count: counts?.services ?? "-", href: ADMIN_ROUTES.SERVICES.LIST, icon: iconComponents.services, description: "Список услуг" },
      { id: 6, label: "Цены", count: counts?.services ?? "-", href: ADMIN_ROUTES.PRICES.MASTER, icon: iconComponents.prices, description: "Прайс-лист" },
      { id: 7, label: "История", count: counts?.history ?? "-", href: ADMIN_ROUTES.APPOINTMENTS.HISTORY, icon: iconComponents.history, description: "Архив записей" },
      { id: 8, label: "Пользователи", count: counts?.users ?? "-", href: ADMIN_ROUTES.USERS, icon: iconComponents.users, description: "Управление аккаунтами" },
      { id: 9, label: "Аналитика", href: ADMIN_ROUTES.ANALYTICS.DASHBOARD, icon: iconComponents.analytics, description: "Отчёты и статистика" },
    ];
    const masterItems: MenuItem[] = [
      { id: 1, label: "Записи", count: counts?.appointments ?? "-", href: MASTER_ROUTES.APPOINTMENTS, icon: iconComponents.appointments, description: "Мои записи" },
      { id: 2, label: "Расписание", count: counts?.schedule ?? "-", href: MASTER_ROUTES.SCHEDULE, icon: iconComponents.schedule, description: "Мой график" },
    ];
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (!target.closest(".mobile-menu") && !target.closest(".mobile-trigger")) {
    ...
    <>
      ...
      <header className="hidden lg:flex fixed top-0 left-0 right-0 z-50 h-16 bg-white/80 backdrop-blur-xl border-b border-gray-200/50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between w-full">
          ...
          {/* Логотип */}
          <Link href={ADMIN_ROUTES.DASHBOARD} className="flex items-center gap-3 group">
            <motion.div
          ...
          <nav className="flex items-center gap-1">
            ...
            <div className="relative" onMouseEnter={() => setIsMenuOpen(true)} onMouseLeave={() => setIsMenuOpen(false)}>
              <button className={`px-3 py-2 rounded-xl text-sm font-medium flex items-center gap-1.5 transition-all
                ${isMenuOpen ? "text-blue-600 bg-blue-50" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100/50"}`}>
                Ещё <ChevronDown size={14} className={`transition-transform ${isMenuOpen ? "rotate-180" : ""}`} />
              </button>
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between">
                                <span className={`font-medium text-sm ${active ? "text-gray-900" : "text-gray-700"}`}>
                                  {item.label}
                                </span>
          ...
          <div className="relative profile-dropdown">
            ...
            >
              ...
              <div className="text-left hidden xl:block">
                ...
                <p className="text-xs text-gray-500">{user.role === "admin" ? "Админ" : "Мастер"}</p>
              </div>
              ...
              {isProfileOpen && (
                ...
                >
                  <div className="p-4 border-b border-gray-100">
                    ...
                    <p className="text-sm text-gray-500">{user.role === "admin" ? "Администратор системы" : "Мастер"}</p>
                  </div>
                  <div className="p-2">
                    <Link href="/profile" className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition-colors">
              ...
              <div className="p-5 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 text-white">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-blue-100/80 text-sm">{isAdmin ? "Администратор" : "Мастер"}</p>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-3 p-3 bg-white/10 rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-100/80">{user.role === "admin" ? "Админ" : "Мастер"}</p>
                  </div>
                </div>
              </div>
